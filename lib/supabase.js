...
export function getSupabaseClient() {
  ...
  // サービスロールキーを優先使用（RLSをバイパス）、なければANON_KEYを使用
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY

  if (!supabaseUrl || !supabaseKey) {
    throw new Error('Supabase credentials are not set. Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_ANON_KEY) in environment variables.')
  }
...
export async function savePage(userId, siteUrl, pageUrl, pageTitle = '') {
  const supabase = getSupabaseClient()
  console.log('[savePage] Saving page:', { userId, siteUrl, pageUrl, pageTitle })
  ...
  if (fetchError && fetchError.code !== 'PGRST116') {
    console.error('[savePage] Fetch error:', fetchError)
    throw fetchError
  }
  ...
  if (existingPage) {
    console.log('[savePage] Updating existing page:', existingPage.id)
    // 既存のページを更新
    const { data, error } = await supabase
      .from('page_tracker_pages')
      .update({
    ...
    console.log('[savePage] Inserting new page')
    // 新しいページを挿入
    const { data, error } = await supabase
      .from('page_tracker_pages')
      .insert({
    ...
    if (error) {
      console.error('[savePage] Insert error:', error)
      throw error
    }
    pageId = data.id
  }

  console.log('[savePage] Success, pageId:', pageId)
  return pageId
}
...
export async function saveDailyData(pageId, dailyData) {
  ...
  const records = dailyData.map(day => ({
    ...
    unique_query_count: day.uniqueQueryCount || 0
  }))
...
export async function getPages(userId, siteUrl) {
  const supabase = getSupabaseClient()
  console.log('[getPages] Fetching pages for:', { userId, siteUrl })
  ...
  console.log('[getPages] Found pages:', pages?.length || 0)
  ...
  return pages.map(page => ({
    ...
    dailyData: page.page_tracker_daily
      .map(d => ({
        ...
        uniqueQueryCount: d.unique_query_count || 0
      }))
...
export async function savePages(userId, siteUrl, pages) {
  console.log('[savePages] Saving pages:', { userId, siteUrl, count: pages.length })
  const results = []

  for (const pageData of pages) {
    try {
      console.log('[savePages] Processing page:', pageData.pageUrl)
      ...
      if (pageData.dailyData && pageData.dailyData.length > 0) {
        console.log('[savePages] Saving daily data for page:', pageId, 'count:', pageData.dailyData.length)
        await saveDailyData(pageId, pageData.dailyData)
      }

      results.push({ success: true, pageId })
      console.log('[savePages] Successfully saved page:', pageData.pageUrl)
    } catch (error) {
      console.error(`[savePages] Failed to save page: ${pageData.pageUrl}`, error)
      results.push({ success: false, error: error.message })
    }
  }

  console.log('[savePages] Complete, results:', results)
  return results
}
